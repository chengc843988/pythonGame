...
var HttpStatusActive=false
const HttpStatus=new XMLHttpRequest();
HttpStatus.onreadystatechange=function() {
  if(HttpStatus.readyState===4) {
  var res=HttpStatus.responseText;
  // console.log(res);
  document.getElementById("esp32Now").innerHTML= res;
    HttpStatusActive=false
  }
}

var esp32Now=window.setInterval( () => {
    if(HttpStatusActive) return;
    HttpStatusActive=true
    var cubeArray=""
    if(solvers.length)
     cubeArray=solvers[solvers.length-1].asString()
    HttpStatus.open("GET","/status?cube="+cubeArray);
    HttpStatus.send();
...
function forwardAction(op,inverse,cubes) {
    var cubeArray=""
    currentCubes=cubes;
    if(solvers.length)
     cubeArray=solvers[solvers.length-1].asString()

    var url="/cubeCommand"+"?op="+op+"&inverse="+(inverse|'false')
    +("&cube="+cubeArray);
    Http.open("GET",url);
    Http.send();
    console.log(url);
    console.log(solvers[solvers.length-1].asString() );
    // console.log(currentCubes.asString());
}
...
function action(cubes, scene, operation, inverse, done) {
    ...
    forwardAction(operation,inverse,cubes);
...
function init() {
    ...
    function fromString(cubes,scene) {
        // var x=solvers[solvers.length-1];
        var x=Cube.fromString("UUUUUUUUURRRRRRRRRFFFFFFFFFDDDDDDDDDLLLLLLLLLBBBBBBBBB");
        solvers.push(x);
        console.log(x.asString());
    }

    if(document.getElementById("fromString")) document.getElementById("fromString").onclick = function() { fromString(cubes, scene); };
